  ...
  each(measures, function (systems, measure) {
    each(systems, function (units, system) {
      ...
      each(units, function (unit, testAbbr) {
        ...
        unit.name.alt = ( unit.name.alt !== undefined )
          ? unit.name.alt
          : ''
        if(testAbbr == abbr 
          || unit.name.singular.toLowerCase() == abbr 
          || unit.name.plural.toLowerCase() == abbr 
          || unit.name.alt.toLowerCase() == abbr ) {
          found = {
            abbr: abbr
          , measure: measure
          , system: system
...
*/
Converter.prototype.list = function (measure, searchSystem) {
  var list = [];
  ...
  each(measures, function (systems, testMeasure) {
    ...
    each(systems, function (units, system) {
      ...
      each(units, function (unit, abbr) {
        if ( ( searchSystem && system == searchSystem ) || ! searchSystem ) {
